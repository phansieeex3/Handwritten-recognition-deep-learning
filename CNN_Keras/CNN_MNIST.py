...
for num in np.random.choice(np.arange(0, len(test_labels)), size=(5,)):
	...
	probs = clf.predict(test_img[np.newaxis, num]).ravel()
...
fpr_keras, tpr_keras, thresholds_keras = roc_curve(test_labels,probs)    
...
conf_mat = confusion_matrix(test_labels.argmax(axis=1),probs.argmax(axis=1))
...
image2 = cv2.merge([image] * 3)
image3 = cv2.resize(image2, (100, 100), interpolation=cv2.INTER_LINEAR)
cv2.putText(image3, str(prediction[0]), (5, 20),cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
